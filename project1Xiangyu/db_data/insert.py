...
for line in reader:
	...
	if 0 == cnt:	
		...
		sql = 'INSERT IGNORE INTO `BOOK` (`ISBN`, `Title`) VALUES ("%s", "%s")' % (line[0], line[2].replace('"',''))
	elif cnt > 1:
	...
	if cnt > 20000:
		...
		try:
			...
			print "IntegrityError in INSERT BOOK table from books.csv"
		sql = ''
...
for line in reader:
	...
	if cnt > 20000:
		...
		try:
			...
			print "IntegrityError in BOOK_AUTHORS table from books.csv"
		sql = ''
...
if len(sql) != 0:
	try:
		...
		print "IntegrityError in INSERT BOOK_AUTHORS table from books.csv"
...
for line in reader:
	if 0 == cnt:	# except first row
		...
		sql = 'INSERT IGNORE INTO BOOK_COPIES (ISBN, Branch_id, No_of_copies) VALUES ("%s", %d, %d)' % \
...
for line in reader:
	if 0 == cnt:	# except first row
		...
		continue
			...
			sql = 'INSERT IGNORE INTO BORROWER (Card_no, Ssn, Fname, Lname, Address, Phone) VALUES ("%s", "%s","%s", "%s", "%s", "%s")' % \
			(line[0], line[1], line[2], line[3], line[5], line[8])
	elif cnt > 1:
			sql += ', ("%s", "%s","%s", "%s", "%s", "%s")' % (line[0], line[1], line[2], line[3],line[5], line[8])
...
# try:
# 	sql = "INSERT INTO BOOK_AUTHORS (Book_id, Author_id) VALUES ('%s', '%s')" % ('12347585', 'Harry Potter')
# 	c.execute(sql)
# except mysql.IntegrityError:
